...
def get_history(user_id):
    ...
    rows = conn.execute("""
        SELECT analyses_id, score_total, score_acne, score_pore,
                score_pigmentation, score_sebum, raw_values, created_at
            FROM skin_analyses
            WHERE user_id = ?
    ...
    for h in history:
        h["raw_values"] = json.loads(h["raw_values"]) if h["raw_values"] else {}
